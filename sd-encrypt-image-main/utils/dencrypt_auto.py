...
def main():
    ...
    output_dir = r"G:\PythonProject\Projects\本地WebUI\图集"
    ...
    for root, dirs, filenames in os.walk('G:\PythonProject\Projects\本地WebUI\加密\kaggle\working'):
        print('发现文件：'+str(len(filenames)))
            ...
            if filename.endswith('.jpg') or filename.endswith('.png') or filename.endswith('.webp') or filename.endswith('.jpeg'): 
                ...
                try:
                    ...
                    if 'Encrypt' in pnginfo and pnginfo["Encrypt"] == 'pixel_shuffle_2':
                        ...
                        pnginfo["Encrypt"] = None
                        jpgfile = os.path.splitext(output_filename)[0]+'.jpg'
                        image.save(jpgfile,quality=95)
                        pnginfo1=pnginfo['parameters']
                        exif_bytes = piexif.dump({
                            "Exif": {
                                piexif.ExifIFD.UserComment: piexif.helper.UserComment.dump(pnginfo1, encoding="unicode")
                            },
                        })
                        piexif.insert(exif_bytes, jpgfile)
                    image.close()
                    print(f"已解密 {filename} 文件")
